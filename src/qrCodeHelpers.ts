...
export const composeQrCode = async ({
	pairingCode,
	setupUri,
}: ComposeQrCode) => {
...
export const parseSetupUri = (setupUri: string): ParsedSetupUri | null => {
	if (!setupUri.startsWith('X-HM://')) {
		return null;
	}

	const data = setupUri.slice(7); // Remove 'X-HM://'
	if (data.length < 9) {
		return null;
	}

	const payloadBase36 = data.slice(0, 9);
	const setupId = data.slice(9);
	...
	try {
		...
		const flag = Number(payload & BigInt(0xf));
		payload = payload >> BigInt(4);

		// Extract categoryId (bottom 8 bits)
		const categoryId = Number(payload & BigInt(0xff));
		payload = payload >> BigInt(8);

		// Extract reserved (bottom 4 bits)
		const reserved = Number(payload & BigInt(0xf));
		payload = payload >> BigInt(4);

		// Extract version (bottom 3 bits)
		const version = Number(payload & BigInt(0x7));
		...
		console.error('Error parsing setup URI:', error);
		return null;
	}
};
